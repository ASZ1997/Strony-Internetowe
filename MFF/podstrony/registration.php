	...
	if((strlen($nazwa)<3) || (strlen($nazwa)>20))
		...
		try		//przechwytywanie wyjątków
		{
			...
			else
			{
				...
				$rezultat = $_SESSION['polaczenie']->query("SELECT id FROM uzytkownicy WHERE email='$email'");
				...
				$rezultat = $_SESSION['polaczenie']->query("SELECT id FROM uzytkownicy WHERE nazwa='$nazwa'");
				if(!$rezultat) throw new Exception($_SESSION['polaczenie']->error);
		...
		if($ok == true) //wszystkie testy zaliczone
		{
			...
			if($_SESSION['polaczenie']->query("INSERT INTO uzytkownicy VALUES(NULL,'$nazwa','$haslo_hash','$email')"))
